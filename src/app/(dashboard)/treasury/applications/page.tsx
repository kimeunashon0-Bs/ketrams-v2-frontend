...
export default function TreasuryApplicationsPage() {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      const res = await api.get('/treasury/applications', { params });
      setApplications(res.data.data);
      const uniqueWards = [...new Set(res.data.data.map((app: Application) => app.student.ward))] as string[];
  ...
  const fetchInstitutions = async () => {
    try {
      const res = await api.get('/treasury/institutions');
  ...
  const fetchCourses = async () => {
    try {
      const res = await api.get('/treasury/courses');
      setCourses(res.data);
    } catch (error) {
  ...
  const exportCSV = async () => {
    try {
      ...
      if (filters.status) params.status = filters.status;
      if (filters.reported !== '') params.reported = filters.reported === 'true';
      const res = await api.get('/treasury/applications/export/csv', {
        params,
        responseType: 'blob',
      });
      ...
      link.href = url;
      link.setAttribute('download', 'treasury-applications.csv');
  ...
  const viewStudentProfile = async (studentId: number) => {
    try {
      const res = await api.get(`/treasury/students/${studentId}`);
